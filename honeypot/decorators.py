...
def verify_honeypot_value_SellForm(request, field_name):
    """
        Verify that request.POST[field_name] is a valid honeypot.
...
def check_honeypot_SellForm(func=None, field_name=None):
    """
        Check request.POST for valid honeypot field.
    ...
    def decorated(func):
        def inner(request, *args, **kwargs):
            response = verify_honeypot_value_SellForm(request, field_name)
            if response:
                return response
            else:
                return func(request, *args, **kwargs)
...
def check_honeypot_QuoteForm(func=None, field_name=None):
    """
        Check request.POST for valid honeypot field.
    ...
    def decorated(func):
        def inner(request, *args, **kwargs):
            response = verify_honeypot_value_QuoteForm(request, field_name)
            if response:
                return response
            else:
                return func(request, *args, **kwargs)
